...
final class BenchmarkListener extends SparkListener {
  ...
  // ── Pamięć ────────────────────────────────────────────────────────────────
  /** executorId -> peak on-heap+off-heap exec memory bytes seen so far. */
  private val peakExecMem = mutable.Map.empty[String, Long]
  ...
  override def onExecutorMetricsUpdate(e: SparkListenerExecutorMetricsUpdate): Unit = {
    val updates = e.executorUpdates
    if (updates == null) return
    updates.values.foreach { metrics =>
      val total = metrics.getMetricValue("JVMHeapMemory") +
                  metrics.getMetricValue("JVMOffHeapMemory")
      val prev  = peakExecMem.getOrElse(e.execId, 0L)
      if (total > prev) peakExecMem.update(e.execId, total)
    }
  }
  ...
  def snapshot(): BenchmarkListener.ListenerSnapshot =
    BenchmarkListener.ListenerSnapshot(
      ...
      peakExecutorMemoryBytes = if (peakExecMem.isEmpty) 0L else peakExecMem.values.max,
