...
import clustering.algorithms.dbscan.components.{CandidateSelectionStrategy, UniformSelection}
...
class CandidateSelectionStrategySpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("fromName resolves uniform and rejects an unknown strategy") {
    assert(CandidateSelectionStrategy.fromName("uniform").strategyName == "uniform")
    intercept[IllegalArgumentException](CandidateSelectionStrategy.fromName("nope"))
  }
}
