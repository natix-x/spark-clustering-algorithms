...
class WeightedClusteringSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("fastpam / fasterpam: weighted medoids equal medoids of the duplicated data") {
    ...
    ).zip(Seq("fastpam", "fasterpam")).foreach { case (solver, name) =>
  ...
  test("dbscanpp: minPts is a mass threshold, so weights decide core points") {
    ...
    val model = new DBSCANpp(eps = 1.0, minPts = 4, coreSampleFraction = 1.0,
      distanceMetric = EuclideanDistance).fit(weighted)
    val coreXs = model.corePoints.map(_(0)).sorted.toSeq
    ...
    val dupModel = new DBSCANpp(eps = 1.0, minPts = 4, coreSampleFraction = 1.0,
      distanceMetric = EuclideanDistance).fit(duplicated)
    assert(dupModel.corePoints.map(_(0)).distinct.sorted.toSeq == coreXs)
    assert(dupModel.coreClusterLabels.distinct.length == model.coreClusterLabels.distinct.length)
  }
}
