...
/** Connected components of the ε-graph over the core points — step 3 of DBSCAN++. Always
 *  distributed (5.09.2026): the driver-local fallback was cut, the distributed path pays its
 *  fixed cost unconditionally.
 */
private[dbscan] object EpsilonGraphComponents {
  ...
  def compute(cores: Array[Vector], eps: Double, metric: DistanceMetric, sc: SparkContext): Array[Int] = {
    ...
    val rowsPerBlock = math.max(MAX_ROWS_PER_DISTRIBUTED_BLOCK, cores.length / MAX_BLOCKS).toInt
    ...
    val labels = computeDistributed(cores, eps, metric, sc, rowsPerBlock)
