...
private[kmedoids] final class NearestMedoidCache(pointCount: Int) {
  ...
  /** Recomputes all three arrays in O(n·k) — parallel over points only when n·k is large enough
   *  to be worth a fork (it usually is not; this is the cheap phase). */
  def refresh(distances: DistanceMatrix, medoids: Array[Int]): Unit = {
    val k = medoids.length
    DriverParallelism.sliceRangesForWork(pointCount, k.toLong).par.foreach { case (from, until) =>
      var point = from
      while (point < until) {
